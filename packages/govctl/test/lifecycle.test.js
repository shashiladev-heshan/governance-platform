...
describe('assistant wiring', () => {
  ...
  test('governed skills are projected into .claude/skills', () => {
    // .governance/ is not a path Claude Code scans, so without this the
    // standards are in the repo and invisible to the tool writing the code.
    const p = join(world.project, '.claude/skills/nestjs-conventions/SKILL.md');
    assert.ok(existsSync(p), 'missing .claude/skills/nestjs-conventions/SKILL.md');
    const skill = readFileSync(p, 'utf8');
    assert.match(skill, /^name: nestjs-conventions$/m);
    assert.match(skill, /thin-controllers/);
    assert.match(skill, /Generated by govctl/);
  });

  test('CLAUDE.md gets a marker-delimited governed block', () => {
  ...
  test('Copilot gets glob-scoped instruction files plus a repo-wide index', () => {
    const instruction = readFileSync(
      join(world.project, '.github/instructions/nestjs-conventions.instructions.md'),
      'utf8',
    );
    assert.match(instruction, /^applyTo: '.*\*\*\/\*\.controller\.ts/m);
    assert.match(instruction, /^description: '/m);
    assert.match(instruction, /thin-controllers/);

    const index = readFileSync(join(world.project, '.github/copilot-instructions.md'), 'utf8');
    assert.match(index, /Coding standards for this repository/);
    assert.match(index, /nestjs-conventions/);
  });

  test('a skill retired upstream stops being taught after sync', () => {
    const registrySkill = join(world.registry, 'skills', 'api-design-patterns');
    rmSync(registrySkill, { recursive: true, force: true });
    cutRelease(world.registry, 'v0.2.0', world.keyPath, world.env);

    assert.equal(govctl(['sync'], { cwd: world.project, env: world.env }).code, 0);
    assert.equal(
      existsSync(join(world.project, '.claude/skills/api-design-patterns/SKILL.md')),
      false,
      'a retired skill must not linger in the assistant wiring',
    );
    assert.equal(
      existsSync(join(world.project, '.github/instructions/api-design-patterns.instructions.md')),
      false,
    );
  });

  test('--skip-assistants opts out entirely', () => {
    const project = join(world.root, 'no-assistants');
    mkdirSync(project, { recursive: true });
    const result = govctl(
      ['init', '--registry', world.registry, '--tier', 'corporate', '--tag', 'v0.1.0', '--skip-assistants'],
      { cwd: project, env: world.env },
    );
    assert.equal(result.code, 0, result.stdout + result.stderr);
